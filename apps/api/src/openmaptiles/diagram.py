...
class GraphGenerator:
    ...
    def compare_file(self, dot_file, ext, new_file):
        if not self.compare_dir:
            return
        cmp_with = Path(self.compare_dir, dot_file.relative_to(self.output_dir)).with_suffix(ext)
        try:
            old = cmp_with.read_bytes()
            new = Path(new_file).read_bytes()
            if old != new:
                raise ValueError(
                    f"file has changed, old size = " f"{len(old):,} B,  new = {len(new):,} B"
                )
...
class EtlGraph(GraphGenerator):
    ...
    def get_graph(self, layer: Layer, is_tileset: bool) -> Tuple[Digraph, Path]:
        raw_lines = self.parse_files(
            layer.imposm_mapping_files, self.re_mapping
    ...
    def get_graph(self, layer: Layer, is_tileset: bool) -> Tuple[Digraph, Path]:
        ...
        for imposm_mapping in layer.imposm_mappings:
            for name, value in imposm_mapping["tables"].items():
                ...
                if mapping:
                    graph.subgraph(self.generate_mapping_subgraph(name, mapping.items()))
