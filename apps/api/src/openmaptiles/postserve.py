...
from asyncpg import Connection, ConnectionDoesNotExistError, PostgresLogMessage, create_pool
...
from openmaptiles.pgutils import show_settings, get_postgis_version, PgWarnings, get_vector_layers
    ...
    async def init_connection(self):
        async with self.pool.acquire() as conn:
            ...
            self.metadata = self.create_metadata(
                [self.url + "/tiles/{z}/{x}/{y}.pbf"], await get_vector_layers(conn, mvt)
            )
