...
def assert_int(
    value, name: str, min_val: Optional[int] = None, max_val: Optional[int] = None, required=False
):
    if value is None:
        if required:
        ...
        raise ValueError(
            f'The {name} value was expected to be an integer, but found {type(value).__name__} "{value}"'
        )
        ...
        if isinstance(definition, str):
            ...
            if values:
                if isinstance(values, dict):
                    ...
                    raise ValueError(f"Field {name} has unexpected type {type(values)} for values")
        elif definition:
...
class Layer:
    ...
    def __init__(
        ...
        self.imposm_mapping_files = [
            Path(layer_dir, ds["mapping_file"])
        ...
        if self.definition["layer"].get("fields"):
            self.fields = [Field(k, v) for k, v in self.definition["layer"]["fields"].items()]
        else:
            self.fields = []
        ...
        err = (
            'If set, "requires" parameter must be a map with optional "layers", "tables", and "functions" sub-elements. Each sub-element must be a string or a list of strings. If "requires" is a list or a string itself, it is treated as a list of layers. '
        ...
        self.requires_tables = get_requires_prop(
            ...
            "tables",
            err
            + '"requires.tables" must be the name of a PostgreSQL table or a view, or a list of tables/views',
        )
        self.requires_functions = get_requires_prop(
            requires,
            "functions",
            err
            + '"requires.functions" must be a PostgreSQL function name with parameters or a list of functions. Example: "sql_func(TEXT, TEXT)"',
        )
        ...
        if requires:
            ...
            raise ValueError(
                f'Unrecognized sub-elements in the "requires" parameter: {str(list(requires.keys()))}'
            )
    ...
    def buffer_size(self) -> int:
        ...
        min_size = assert_int(
            self.definition["layer"].get("min_buffer_size"), "min_buffer_size", min_val=0
        )
        if size is None and min_size is None:
            raise ValueError(
                f'Layer "{self.id}" is missing an integer buffer_size and/or min_buffer_size'
            )
        ...
        if self.tileset:
            val = assert_int(
                self.tileset.overrides.get("buffer_size"), "buffer_size global override", min_val=0
            )
            if val is not None:
                size = val
        ...
        if self.overrides:
            val = assert_int(
                self.overrides.get("buffer_size"), "buffer_size layer override", min_val=0
            )
            min_val = assert_int(
                self.overrides.get("min_buffer_size"), "min_buffer_size layer override", min_val=0
            )
            if val is not None and min_val is not None and val < min_val:
                raise ValueError(
                    f'Layer overrides for "{self.id}" have buffer_size less than min_buffer_size'
                )
            if val is not None:
                size = val
...
class Tileset:
            ...
            for lid, layer in list(unresolved.items()):
                for req in layer.requires_layers:
                    if req not in self.layers_by_id:
                        raise ValueError(f"Unknown layer '{req}' required for " f"layer {layer.id}")
    ...
    def languages_as_fields(self) -> List[str]:
        ...
        """
        return [f"name:{lang}" for lang in self.languages] + Tileset.auto_language_fields
...
def process_layers(filename: Path, processor: Callable[[Layer, bool], None]):
    ...
    if "tileset" in parsed.data:
        ...
        raise ValueError(
            f"Unrecognized content in file {filename} "
