...
class MvtGenerator:
    ...
    def __init__(
        ...
        m = re.match(
            r"^(?P<major>\d+)\.(?P<minor>\d+)" r"(\.(?P<patch>\d+)(?P<suffix>[^ ]*)?)?", ver
        )
        if not m:
...
as mvtl{extras} FROM {query}"""
    ...
    async def validate_layer_fields(
        ...
        if layer_fields != query_fields:
            ...
            query_fields -= same
            error = (
                f"Declared fields in layer '{layer_id}' do not match "
                f"the fields received from a query:\n"
            )
            if layer_fields:
                error += (
                    f'  These fields were declared, but not returned by '
                    f"the query: {', '.join(layer_fields)}"
                )
            if query_fields:
                error += (
                    f'  These fields were returned by the query, '
