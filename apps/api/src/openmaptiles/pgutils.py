...
async def get_postgis_version(conn: Connection) -> str:
    try:
        ...
        raise ValueError(
            "postgis_full_version() does not exist, " "probably because PostGIS is not installed"
        )
...
async def show_settings(conn: Connection, verbose=True) -> Dict[str, str]:
    settings = {
        ...
        "jit": lambda v: "disable JIT in PG 11+ for complex queries" if v != "off" else "",
        "shared_buffers": None,
        "work_mem": None,
...
def parse_pg_args(args, legacy_params=False):
    ...
    pghost = coalesce(
        tmp_host, args.get("--pghost"), getenv("POSTGRES_HOST"), getenv("PGHOST"), "localhost"
    )
    pgport = coalesce(
        tmp_port, args.get("--pgport"), getenv("POSTGRES_PORT"), getenv("PGPORT"), "5432"
    )
    dbname = coalesce(
        tmp_db, args.get("--dbname"), getenv("POSTGRES_DB"), getenv("PGDATABASE"), "openmaptiles"
    )
    user = coalesce(args.get("--user"), getenv("POSTGRES_USER"), getenv("PGUSER"), "openmaptiles")
    password = coalesce(
        args.get("--password"), getenv("POSTGRES_PASSWORD"), getenv("PGPASSWORD"), "openmaptiles"
    )
...
async def get_vector_layers(conn, mvt) -> List[dict]:
    ...
    for layer_id, layer in mvt.get_layers():
        ...
        unknown = {name: oid for name, oid in fields.items() if oid not in pg_types}
        if unknown:
            print(
