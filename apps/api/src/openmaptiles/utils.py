...
class Bbox:
    def __init__(
        self, bbox=None, left=-180.0, bottom=-85.0511, right=180.0, top=85.0511, center_zoom=5
    ) -> None:
        if bbox:
    ...
    def from_polygon(content: str):
        ...
        return Bbox.from_geometry([[float(v[0]), float(v[1])] for v in coords if len(v) == 2])
        ...
        for action_id in list(pending):
            ...
            for dep in list(action.depends_on):
                if dep not in lookup:
                    msg = f"Action '{action.action_id}' depends " f"on an undefined action '{dep}'"
...
def batches(
    items: Iterable[T], batch_size: int, decorator: Callable[[T], T2] = lambda v: v
) -> Iterable[List[T2]]:
...
def parse_tags(feature: TileFeature, layer: TileLayer, show_names: bool, summary: bool) -> dict:
    if summary:
        show_names = True
    ...
    tags = {
        layer.keys[feature.tags[i]]: which_one_of(layer.values[feature.tags[i + 1]], "val")[1]
...
def print_tile(
    data: bytes, show_names: bool, summary: bool, info: str, sort_output: bool = False
) -> None:
    ...
    print(
        f"{info} size={len(tile_raw):,} bytes, gzipped={gzipped_size:,} bytes, {len(tile.layers)} layers"
    )
    res = []
    layers = tile.layers
    if sort_output:
    ...
    for layer in layers:
        ...
        if summary:
            ...
            entry = {
                ...
                "Fields (percentage only if not all features have it)": format_stats(tag_stats),
            }
            if name_stats:
                if show_names:
                    entry["name:* fields (percentage of features with that language)"] = (
                        format_stats(name_stats, True)
                    )
                else:
            ...
            if not show_names:
                ...
                if hidden_names:
                    extra = f", hiding {len(hidden_names)} name:* languages: " + ",".join(
                        hidden_names
                    )
            print(
                f"\n======= Layer {layer.name}: "
...
def parse_zoom_list(
    zoom: Union[None, str, List[str]], minzoom: Optional[str] = None, maxzoom: Optional[str] = None
) -> Optional[List[int]]:
...
def parse_zoom(
    zooms: Union[None, str, List[str]], is_list: bool = False
    ...
    if not is_list and len(zooms) > 1:
        raise ValueError(
            f"One zoom value was expected, but multiple values were given: [{', '.join(zooms)}]"
        )
