...
class KeyFinder:
    ...
    def run(self):
        ...
        with sqlite3.connect(self.mbtiles) as conn:
            ...
            if self.show_size:
                sql = (
                    "SELECT cnt, dups.tile_id, LENGTH(tile_data) FROM ("
        ...
        for vals in results:
            ...
            if self.show_examples:
                example_sql = (
                    "SELECT zoom_level, tile_column, tile_row FROM map " "WHERE tile_id = ? LIMIT 5"
                )
                examples = [f"{z}/{x}/{y}" for z, x, y in query(conn, example_sql, [tile_id])]
            if self.verbose:
...
class Imputer:
    def __init__(self, mbtiles, keys, zoom, outfile: str = None, verbose=False) -> None:
        self.mbtiles = mbtiles
...
class Metadata:
    def __init__(self, mbtiles: str, show_json: bool = False, show_ranges: bool = False) -> None:
        self.mbtiles = mbtiles
    ...
    def print_all(self, file: str = None):
        ...
        if data:
            ...
            for name, value in sorted(data.items(), key=lambda v: v[0] if v[0] != "json" else "zz"):
...
"""
    ...
    async def generate(self, tileset, reset, auto_minmax, pghost, pgport, dbname, user, password):
    ...
    def _update_metadata(self, metadata, auto_minmax, reset, file, center_zoom=None):
        ...
        if bbox_str:
            bbox = Bbox(bbox=bbox_str, center_zoom=getenv("CENTER_ZOOM", center_zoom))
    ...
    def _get_metadata(file) -> Dict[str, str]:
        with sqlite3.connect(file) as conn:
            return {k: v for k, v in query(conn, "SELECT name, value FROM metadata", [])}
